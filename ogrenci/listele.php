  ...
  if ($_POST) {
    $sonid=$_POST["sonid"];
      ...
      while ($sonuc=mysqli_fetch_array($sorgu)) {
        ...
        $goruldu_update = mysqli_query($baglanti,"UPDATE mesajlar SET okundu=1 WHERE (alici_id=$alici_id AND gonderici_id=$gonderici_id)OR(alici_id=$gonderici_id AND gonderici_id=$alici_id)");
