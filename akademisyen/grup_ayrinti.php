<?php
 ...
 if ($_GET) {
   ...
   /*Paylaşım Yap Son*/
   /*Takip İsteği Onaylama*/
   if (@$_GET["onayla"]) {
     $kisi_id= $_GET["kisi_id"];
     mysqli_query($baglanti,"UPDATE grupuyeleri SET izin=2 WHERE grup_id=$grup_id AND kullanici_id=$kisi_id");
   }
   /*İstek Son*/
    ...
    <div class="duyuru">
        <?php echo '<img src="../images/gruplogolari/'.$grup["grup_logo"].'" class="grup_logo col-md-offset-2" width="270" height="220"/>'; ?>
            ...
            <div class="">
              ...
              if($grup["grup_kuran_id"]==$id){
            ?>
              <a data-toggle="modal" href="#takipIsteklerimodal" class="btn btn-outline-info my-2 my-sm-0 col-md-12  grupUyeleri" id="takipIstekleri" style="color:#7e8880;border-color:#7e8880;margin-top:10px !important;">Takip İstekleri</a>
                <div class="modal fade" id="takipIsteklerimodal" role="dialog">
                   <div class="modal-dialog">
                     <div class="modal-content">
                       <div class="modal-header">
                         <h6>Takip İstekleri</h6>
                         <button type="button" class="close" data-dismiss="modal">&times;</button>
                       </div>
                       <div class="modal-body" style="float:left">
                         <?php
                          $istekler = mysqli_query($baglanti,"SELECT * FROM `grupuyeleri` INNER JOIN kullanicilar ON grupuyeleri.kullanici_id = kullanicilar.kullanici_id WHERE grupuyeleri.grup_id = $grup_id AND grupuyeleri.izin=1");
                          while ($kisileriGetir=mysqli_fetch_array($istekler)) {

                                    echo '<a href="profilayrinti.php?kullanici_id='.$kisileriGetir["kullanici_id"].'" class="link" ><img src="../profilfotograflari/'.$kisileriGetir["profilfotografi"].'" class="profil_foto3" width="60" height="60" style="margin-left:-90px;"/>
                                      '.$kisileriGetir["adSoyad"].'<span style="font-size:12px;" title="'.$kisileriGetir["eMail"].'"> (('.substr($kisileriGetir["eMail"],0,10).'. . .))</span></a>
                                      <a href="grup_ayrinti.php?grup_id='.$grup_id.'&onayla=1&kisi_id='.$kisileriGetir["kullanici_id"].'" class="takipEt" style="float: right;margin-top:10px;margin-right:20px;">Onayla</a><br><br>
                                      <hr><br>';
                            }
                          ?>
